    ...
    def __init__(self, user: discord.Member, title="Leave Modal"):
        ...
        self.add_item(
            ui.InputText(
                ...
                label="Confirm your decision",
                required=True,
                placeholder="Write 'leave' to leave this server.",
            )
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        if not self.children[0].value == "leave":
            return await interaction.response.send_message(
                "You didn't pass the check to leave this server"
            )
        await interaction.response.send_message(
            f"{interaction.user.name}#{interaction.user.discriminator} left whilst the verification."
        )
        await interaction.user.kick(
            reason=f"{interaction.user.name}#{interaction.user.discriminator} left whilst the verification."
        )
