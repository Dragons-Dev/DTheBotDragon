...
class UserCog(commands.Cog):
    ...
    async def userinfo(self, ctx: discord.ApplicationContext, user: discord.Option(
        ...
        em.add_field(name = f"Accout created", value = f"{user.created_at.strftime('%d.%m.%Y %H:%M')}\n<t:{int(time.mktime(user.created_at.timetuple()))}:R>")
        ...
        for role in ctx.author.roles:
            if int(team_id[0]) == int(role.id):
                ...
                for item in response:
                    ...
                    em.add_field(name = f"{action.capitalize()} (ID: {pub_id})",
                                 value = f"**---Reason---**\n{reason}\n------------\nBy <@{moderator}>\nAt {date}",
                                 inline = False)
                    counter += 1
                    if counter > 24:
