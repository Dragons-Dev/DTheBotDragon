...
class BanCog(commands.Cog):
    ...
    async def ban(
        ...
        user: discord.Option(
            ...
            description="mention a user to ban him",
        ),
        ...
        for role in ctx.author.roles:
            if int(team_id[0]) == int(role.id):
                ...
                await db.add_mod_action(
                    member=user.id, moderator=ctx.author.id, reason=reason, action="ban"
                )
                if log is None:
            ...
            await ctx.response.send_message(
                f"You are not allowed to ban a user.", ephemeral=True
            )
