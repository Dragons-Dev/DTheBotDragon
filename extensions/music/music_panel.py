...
class MusicPanel(commands.Cog):
    ...
    @commands.slash_command(name = "setup_panel", description = "Summon the new music panel to use")
    async def setup_panel(self, ctx: discord.ApplicationContext):
        ...
        for role in ctx.author.roles:
            if int(team_id[0]) == int(role.id):
                ...
                msg = await ctx.channel.send(embed = discord.Embed(title = "No song playing now",
                                                                   description = "Add new tracks by using /play",
                                                                   color = discord.Color.blurple()))
                await db.insert_setting(setting = "music_panel",
                                        value = str(msg.id),
                                        guild = ctx.guild_id)
                await db.insert_setting(setting = "music_panel_channel",
