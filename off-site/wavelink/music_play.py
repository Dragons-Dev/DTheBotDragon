...
class MusicPlay(commands.Cog):
    ...
    async def music_play(
        ...
        message_id = await db.get_setting(setting="music_panel", guild=ctx.guild.id)
        channel_id = await db.get_setting(
            setting="music_panel_channel", guild=ctx.guild.id
        )

        msg = await utils.fetch_or_get_message(
            client=self.client,
